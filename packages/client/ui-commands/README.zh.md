...
paletteEntries(session, signal) 将同一批来源折叠为面板面行，供斜杠 token 机制之外的面板式界面使用（参考消费者是 dsh-client-ui-command-palette）：裸宿主行（带前置参数的命令标记 rgsRequired——参数认领仍归输入框所有）、可用装饰将其宿主行替换为 popup 形态、按可用性过滤的贡献加入；贡献与宿主名冲突时与候选合成一致地响亮失败。被寻址的子代理会话经目录加载器返回空名单。
