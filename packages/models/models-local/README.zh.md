...
`ctx.models` 的本地 llama.cpp provider：目录就是一个 `.gguf` 权重目录，经 GGUF 头部读取器读出；`hardware()` 提供 `dsh-hardware-probe` 的一次性缓存探测；加载／卸载经 `ctx.subprocess` 驱动被生成的 `llama-server`，配合有界 `/health` 轮询与空闲端口分配。下载属于任务 E3 的切片，这里响亮拒绝。
...
- dispose 尽力卸载全部；卡死的孩子随 subprocess 服务自身的拆卸升级消亡。
...
- **无下载能力**——在 E3 的下载管理器切片落地前，`startDownload`／`downloads()` 响亮拒绝。
