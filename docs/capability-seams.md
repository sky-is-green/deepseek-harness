...
| `ctx.models` | `seam` | [`models`](../packages/models/models) | - | `ui-models-manager` | - | Provider plugins implement catalog, hardware summaries, load state, and the download lifecycle behind one typed service; client surfaces consume the same event stream. |
