...
flowchart LR
  ...
  pkg_models["models"]
  svc_models["ctx.models<br/>Local model hosting seam"]
  pkg_ui_models_manager["ui-models-manager"]
  ...
  svc_models --> pkg_ui_models_manager
...
| `ctx.models` | `seam` | [`models`](../packages/models/models) | - | `ui-models-manager` | - | Provider 插件在一个类型化服务之后实现目录、硬件摘要、加载状态与下载生命周期；客户端表面消费同一事件流。 |
